...
- **alias**: 바깥 JSON 은 `missionType`(camelCase), Python 내부 변수는 `mission_type`(snake_case)로 받습니다. Unity/Backend 규약(camelCase)과 Python 관례(snake_case)를 둘 다 만족시키는 장치.
- **`populate_by_name=True`**: 코드에서 객체를 만들 때 `mission_type=`(필드명)로도, `missionType`(alias)로도 만들 수 있게 허용.
- AI 서버는 `storyId` / `sceneId` 를 받지 않습니다. 판정에 필요한 건 `missionType` + `poseFrames` 뿐입니다.
...
def check_mission(request):
    ...
    if detect is None:                                # 2) 미등록 미션
        return MissionCheckResponse(..., error_code="UNKNOWN_MISSION_TYPE")
    return detect(request.pose_frames)                # 3) detector 실행
```

- **레지스트리 패턴**: `if/elif` 대신 dict 로 missionType→함수 매핑. 미션이 늘어나도 dict 에 한 줄 추가하면 됩니다.
- 여기서 처리하는 건 "어느 detector 도 공통으로 막아야 하는 것"(빈 데이터, 미등록 missionType)뿐. 좌표 기반 세부 판정은 detector 로 위임.
- `storyId` / `sceneId` 조합 검증은 AI 서버 책임이 아닙니다 (백엔드 담당).
...
1. `app/core/constants.py` — 새 `MISSION_*`, `REASON_*` 상수 추가
