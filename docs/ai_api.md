...
> **역할 분리 (중요)**
> - **Unity**: 책장이 포물선/반원처럼 넘어가는 **시각적 연출** 담당
> - **AI Server**: 사용자가 **손 하나를 옆으로 충분히 쓸어 넘겼는지**만 판정
>
> skip_book 은 사용자가 책장을 넘기듯 손을 옆으로 쓸어 넘기는 동작을 판정합니다.
> AI Server 는 **실제 포물선/반원 형태를 필수로 검사하지 않습니다.** 손의 좌우 이동폭(xRange)과
> 전체 이동량(totalMovement)을 기준으로 책 넘김을 인정합니다. (곡선 연출은 Unity 담당)
> 세로 곡선(yRange)은 보조 가점으로만 쓰이고, 부족해도 실패시키지 않습니다.
